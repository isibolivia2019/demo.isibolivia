...
$datos = $modelo->sucursalEspecifico($datos);

$registrosNotificaciones = new RegistrosNotificaciones();
$registrosNotificaciones->agregarRegistro($_SESSION['codigo'], "Se genero un reporte de Ventas de ".$nombreMes." ".$año." de la Sucursal ".$datos[0]['nombre_sucursal']);
...
for($i=0;$i<sizeof($lista);$i++){
	...
	$pdf->Row(array(($i + 1),
		...
		utf8_decode('Bs. '.$res),
		utf8_decode('Bs. '.($res - $lista[$i]['compra_unit_producto'])),
		utf8_decode('Bs. '.($lista[$i]['compra_unit_producto'] * $lista[$i]['cant_venta_producto'])),
		utf8_decode('Bs. '.$lista[$i]['total_venta_producto']),
		utf8_decode('Bs. '.(($res - $lista[$i]['compra_unit_producto']) * $lista[$i]['cant_venta_producto'])),
...
$style = array("","","", "B", "", "","","","B", "B", "B", "");
$pdf->SetWidths(array(10,20,25,17,20,25,25,25,25,25,23,22));
$empty = array("","","", $totalCant." Uds.", "", "","","", 'Bs. '.$totalCosto, 'Bs. '.$totalPrecio, 'Bs. '.$totalGanancia, "");
$pdf->FancyRow($empty, $border, $align, $style);
