...
if($sucursal == "todos"){
	...
	$nombreAlmacenamiento = "de todas los puntos de Almacenamiento";
}else{
	$datos = array($sucursal, $mes, $año);
	$modelo = modelo('Producto');
	$lista = $modelo->reporteListaCompraProductosAlmacenamiento($datos);

	$pos = strpos($sucursal, "ALM");
    if($pos === false){
        $datos = array($sucursal);
        $modelo = modelo('Sucursal');
        $sucursalOrigen = $modelo->sucursalEspecifico($datos);
        $nombreAlmacenamiento = "de la Sucursal ".$sucursalOrigen[0]['nombre_sucursal'];
    }else{
        $datos = array($sucursal);
        $modelo = modelo('Almacen');
        $almacenOrigen = $modelo->almacenEspecifico($datos);
		$nombreAlmacenamiento = "del Almacen ".$almacenOrigen[0]['nombre_almacen'];
    }
...
$registrosNotificaciones->agregarRegistro($_SESSION['codigo'], "Se genero un reporte de Compras de ".$nombreMes." ".$año.", ".$nombreAlmacenamiento);
...
for($i=0;$i<sizeof($lista);$i++){
	...
	$pdf->Row(array(($i + 1),
		...
		utf8_decode($lista[$i]['cod_producto']),
		utf8_decode($lista[$i]['nombre_producto']),
		utf8_decode($lista[$i]['cantidad_compra_producto']." Uds."),
		utf8_decode('Bs. '.($lista[$i]['precio_unit_compra_producto'] * $lista[$i]['cantidad_compra_producto'])),
