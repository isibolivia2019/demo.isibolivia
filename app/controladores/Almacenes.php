...
function agregarAlmacen(){
    ...
    $datos = array($codigo, $nombre, $direccion, $telefono);
    $modelo = modelo('Almacen');
    $resp = $modelo->agregarAlmacen($datos);
    $registrosNotificaciones = new RegistrosNotificaciones();
    $registrosNotificaciones->agregarRegistro($_SESSION['codigo'], "Una nuevo Almacen con Nombre:".$nombre.", fue registrado en el Sistema.");
    $data = ['resp' => $resp];
    echo json_encode($data);
}
...
function actualizarAlmacen(){
    ...
    $registrosNotificaciones = new RegistrosNotificaciones();
    $registrosNotificaciones->agregarRegistro($_SESSION['codigo'], "Los datos del Almacen ".$nombre." fue modificado y actualizado, Se recomienda revisar los datos");
    $data = ['resp' => $resp];
    echo json_encode($data);
}
