...
function agregarSucursal(){
    ...
    $datos = array($codigo, $nombre, $direccion, $telefono, $imagen);
    $modelo = modelo('Sucursal');
    $resp = $modelo->agregarSucursal($datos);
    $registrosNotificaciones = new RegistrosNotificaciones();
    $registrosNotificaciones->agregarRegistro($_SESSION['codigo'], "Una nueva Sucursal con Nombre:".$nombre.", fue registrado en el Sistema.");
    $data = ['resp' => $resp];
    echo json_encode($data);
}
...
function actualizarSucursal(){
    ...
    $datos = array($nombre,$direccion,$telefono,$codigo);
    $modelo = modelo('Sucursal');
    $resp = $modelo->actualizarSucursal($datos);
    $registrosNotificaciones = new RegistrosNotificaciones();
    $registrosNotificaciones->agregarRegistro($_SESSION['codigo'], "Los datos de la Sucursal ".$nombre." fue modificado y actualizado, Se recomienda revisar los datos");
    $data = ['resp' => $resp];
    echo json_encode($data);
}
